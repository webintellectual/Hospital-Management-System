...
class Pat_Signup(tk.Toplevel):
    def __init__(self, master=None):
        ...
        self.title("SIGN UP - PATIENT (pat_signup.py)")
...
#----------------------------------------------------Buttons-----------------------------
        ...
        self.createbutton = Button(self,image=self.photo,borderwidth=0,highlightthickness=0,command=lambda: file_manager.sign_up(self, name_entry.get(),username_entry.get(),passwrd_entry.get(),mob_entry.get(), 'patient'))
        ...
        #             with open('database/patient.csv', 'a', newline='') as csv_file:
        #                 fieldnames = ['pat_name', 'username', 'password', 'mobile_no']
        #                 writer = csv.DictWriter(csv_file, fieldnames=fieldnames)
        #                 writer.writerow({'pat_name': name, 'username': username, 'password': password, 'mobile_no': mobile})

        #             with open('database/pat_details.csv', 'a', newline='') as csv_file:
        #                 fieldnames = ['username', 'name', 'age', 'gender', 'email', 'phone', 'blood_grp', 'password'] 
        #                 writer = csv.DictWriter(csv_file, fieldnames=fieldnames)
        #                 writer.writerow({'name': name, 'username': username, 'password': password, 'phone': mobile, 'age': 'N/A', 'gender': 'N/A', 'email': 'N/A', 'blood_grp': 'N/A'})
