...
class doctor_Profile(tk.Toplevel):
    def __init__(self,username,password, master=None):
        ...
        self.title("DOCTOR-PROFILE (doctor_profile.py)")

        self.canvas = tk.Canvas(self, width=window_width, height=window_height)
        self.canvas.pack(fill=tk.BOTH, expand=True)

        image = Image.open('Images/doctor_main_profile.png')
...
#--------------------------------------------- Creating Labels / Buttons --------------------------------------------------------
        ...
        specialization_label = tk.Label(self,text=specialization,font=(inter_font,26),bg="#9FBFC1")
        specialization_label.place(x=535,y=115+(37*6)+(17*6))
...
#--------------------------------------------------------------------Functions defined -----------------------------------------------------------------------------------
    ...
    def edit_button(self,username,password):
        from doctor_profile_update import Emp_Profile_update
        self.withdraw()
        edit = Emp_Profile_update(username,password)
        edit.mainloop()

# if __name__ == "__main__":
#     app = doctor_Profile("Demo123","Demo123")
#     app.mainloop()